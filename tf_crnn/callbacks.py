...
class CustomSavingCallback(Callback):
    ...
    def _export_model(self, logs):
        ...
        with open(os.path.join(folder, CONST.LAYERS_FILENAME), 'w') as f:
            json.dump(model_json, f)

        # model weights
        self.model.save_weights(os.path.join(folder, CONST.MODEL_WEIGHTS_FILENAME))

        # optimizer weights
        optimizer_weights = tf.keras.backend.batch_get_value(self.model.optimizer.weights)
        with open(os.path.join(folder, CONST.OPTIMIZER_WEIGHTS_FILENAME), 'wb') as f:
            pickle.dump(optimizer_weights, f)

        # learning rate
        learning_rate = self.model.optimizer.learning_rate
        with open(os.path.join(folder, CONST.LEARNING_RATE_FILENAME), 'wb') as f:
            pickle.dump(learning_rate, f)

        # n epochs
        epoch = self._current_epoch + 1
        with open(os.path.join(folder, CONST.EPOCH_FILENAME), 'wb') as f:
            pickle.dump(epoch, f)
...
class CustomLoaderCallback(Callback):
    ...
    def set_model(self, model):
        ...
        # Load model weights
        self.model.load_weights(os.path.join(self.loading_dir, CONST.MODEL_WEIGHTS_FILENAME))

        # Load optimizer params
        with open(os.path.join(self.loading_dir, CONST.OPTIMIZER_WEIGHTS_FILENAME), 'rb') as f:
            optimizer_weights = pickle.load(f)
        with open(os.path.join(self.loading_dir, CONST.LEARNING_RATE_FILENAME), 'rb') as f:
...
class CustomPredictionSaverCallback(Callback):
    ...
    def on_predict_begin(self, logs=None):
        ...
        timestamp = str(int(time.time()))
        self._prediction_filename = os.path.join(self.saving_dir, CONST.PREDICTIONS_FILENAME_FORMAT.format(timestamp))

    def on_predict_batch_end(self, batch, logs):
        logits, seq_len, file_names = logs[CONST.BATCH_OUTPUT_LOGS_KEY]
        ...
        with open(self._prediction_filename, 'ab') as f:
            for n, s in zip(file_names, strings):
                n = n[0]  # n is a list of one element
                f.write((n.decode() + ';' + s + '\n').encode(CONST.FILE_ENCODING))
...
class LRTensorBoard(TensorBoard):
    ...
    :ivar log_dir: path to directory to save logs
    :vartype log_dir: str
    """
    ...
    def on_epoch_end(self,
                     epoch,
                     logs=None):
        logs.update({CONST.LEARNING_RATE_LOGS_KEY: tf.keras.backend.eval(self.model.optimizer.lr)})
